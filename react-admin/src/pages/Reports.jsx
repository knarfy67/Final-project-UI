...
const DashBoard = () => {
  ...
  const [userDataStudentDone, setUserDataStudentDone] = useState({
    labels: [],
    datasets: [{
      label: "Staff Accounts Created",
      data: [],
  ...
  const [userDataStudentNot, setUserDataStudentNot] = useState({
    labels: [],
    datasets: [{
      label: "Staff Accounts Created",
      data: [],
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(students)) {
            ...
            setUserDataStudentDone(doneStudents);
            setUserDataStudentNot(notDoneStudents);
  ...
  useEffect(() => {
   ...
   const data = Object.values(staffCountByDay);
    ...
    const studentCountByDayDone = fetchStudent.reduce((acc, staff) => {
      const date = moment(staff.created_at).format('YYYY-MM-DD');
      acc[date] = (acc[date] || 0) + 1;
      return acc;
    }, {});
    ...
    const studentCountByDayNot = fetchStaff.reduce((acc, staff) => {
      const date = moment(staff.created_at).format('YYYY-MM-DD');
      acc[date] = (acc[date] || 0) + 1;
      return acc;
    }, {});

   const labelsDone = Object.keys(studentCountByDayDone);
   const dataDone = Object.values(studentCountByDayDone);

    const labelStudentNot = Object.keys(studentCountByDayNot);
    const dataStudentNot = Object.values(studentCountByDayNot);
    ...
    setUserDataStudentNot({
      labels: labelStudentNot,
      datasets: [{
        label: "Student Accounts Created",
        data: dataStudentNot
      }]
...
const filteredDataStaff = fetchStaff.filter(item =>
  ...
  <h1 className="poppins-semibold text-2xl text-black text-center mt-5">Reports</h1>
  <div className='flex mx-10 justify-around mt-5 text-center'>
     <div className='bg-slate-900 w-44 h-36 rounded-xl'>
      <h1 className='text-white poppins-semibold mt-3 '>Staff Account</h1>
      <p className='text-white'>{fetchStaff.length}</p>
     </div>
     <div className='bg-lime-900 w-44 h-36 rounded-xl'>
     <h1 className='text-white poppins-semibold mt-3 '>Student Account</h1>
      <p className='text-white'></p>
     </div>
     <div className='bg-sky-900 w-44 h-36 rounded-xl'>
     <h1 className='text-white poppins-semibold mt-3 '>Student done Requirements</h1>
      <p className='text-white'>{fetchStaff.length}</p>
     </div>
     <div className='bg-red-900 w-44 h-36 rounded-xl'>
     </div>
  </div>
  {/* <div className='w-3/4 mx-auto mt-2'>
  <h1 className="poppins-semibold text-2xl text-black text-center">Staff All Accounts</h1>
  <LineChart chartData={userData} />
      ...
      <div className='w-1/2'>
      <h1 className="poppins-semibold text-2xl text-black text-center">Student All Accounts</h1>
        <LineChart chartData={userDataStudent} />
      </div>
    </div>
    <div className='flex gap-5 mx-10 my-10'>
    <div className='w-1/2'>
      <h1 className="poppins-semibold text-2xl text-black text-center">Students Done Requirements</h1>
        <LineChart chartData={userDataStudentDone} />
      </div>
      <div className='w-1/2'>
      <h1 className="poppins-semibold text-2xl text-black text-center">Students Lack Requirements</h1>
        <BarChart chartData={userDataStudentNot} />
      </div>
      </div>
