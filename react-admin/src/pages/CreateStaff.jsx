...
const CreateStaff = () => {
    ...
    const Show = show ? (<div className='bg-green-600 rounded mx-40 mt-5'><h1 className='text-white ml-2 p-2 '>{message}</h1></div>) : null;
    const errorShow = showErrors ? (<div className='flex flex-col bg-red-600 rounded mx-40 mt-5'><h1 className='text-white ml-2 p-2'>{messageErrors}</h1></div>) : null;
    ...
    const handleRegister = async(event) => {
        ...
        try {
            const response =  await axios.post('http://localhost:8000/api/staff/add-staff', formData );
            ...
            if (error.response) {
                ...
                setMessageErrors(errorMessages);
                setShowErrors(true)
            } else {
    ...
    return (
        <div className='grid grid-cols-6 mt-3 mx-2'>
            ...
            <Link to="/accounts-dashboard"><div className='flex items-center ml-36 mt-2'>
                ...
                <h1 className='poppins-semibold text-xl'>Register Staff</h1> 
                <input 
                    type="text" 
