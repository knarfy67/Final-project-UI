...
const EditStudent = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await axios.get(`http://localhost:8000/api/student/update-student/${id}`);
            setidNumber(response.data.student.id_number);
    ...
    const handleUpdate = async(event) => {
        ...
        try {
            const response =  await axios.post(`http://localhost:8000/api/student/update-student/${id}`, formData );
    ...
    return (
        <div className='grid grid-cols-6 mt-3 mx-2'>
            ...
            <div className='col-span-5 bg-sky-100 rounded-3xl min-h-screen overflow-y-auto' style={{ maxHeight: 'calc(100vh - 64px)', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                <Link to="/dashboard"><div className='flex items-center ml-36 mt-2'>
                     <IoMdArrowRoundBack />
                    <p className=''> Back</p>
                    </div> </Link>
                {errorShow}
                <div className='flex flex-col mx-40 mt-10 gap-4'>
                <h1 className='poppins-semibold text-xl'>Update Student</h1> 
                <input 
                    type="text" 
