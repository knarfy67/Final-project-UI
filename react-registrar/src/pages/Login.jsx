...
function Login() {
    ...
    const errorLogin = showErrors?  (<div className='bg-red-600 rounded my-2'><h1 className='text-white ml-2 p-2 '>{messageErrors}</h1></div>) : null;
    ...
    const handleLogin = async(event) => {
        ...
        setShowErrors(false);
        try{
          const response =  await axios.post('http://localhost:8000/api/staff/login', {email, password});
          const staff = response.data.staff.staff
          if( "Registrar" === staff) {
            localStorage.setItem("authToken", response.data.token);
            localStorage.setItem("authAdmin", JSON.stringify(response.data.staff));
            navigate("/dashboard");
            setLoading(false);
          }
          else{
            setMessageErrors("invalid account");
            setLoading(false);
            setShowErrors(true)
            console.log("test")
          }
  ...
  return (
    <div className='max-w-6xl mx-auto'>
        ...
        <div className='col-span-1 flex flex-col justify-center mx-6'>
            ...
            { errorShow } {errorLogin}
            <h1 className="mb-4 poppins-regular text-xl">Registrar Login</h1>
